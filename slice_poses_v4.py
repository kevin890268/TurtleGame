...
def discover_v32_action_sheets(species: str) -> list[dict]:
    """
    Discover canonical v3.2 sheets.
    ...
    Also accepts:
        01_walk_a.png
        01_walk_a_4x4.png
        action_01_walk_a_4x4.png
    """
    specs = []

    for action_id, (action_key, action_name) in enumerate(STANDARD_32, start=1):
        candidates = [
            ROOT / f"reference/sheets/{species}/action_{action_id:02d}_{action_key}.png",
            ROOT / f"reference/sheets/{species}/{action_id:02d}_{action_key}.png",
            ROOT / f"reference/sheets/{species}/action_{action_id:02d}_{action_key}_4x4.png",
            ROOT / f"reference/sheets/{species}/{action_id:02d}_{action_key}_4x4.png",
        ]

        found = next((p for p in candidates if p.exists()), None)
        if found:
            spec = make_action_sheet_spec(
                species, action_id, action_key, action_name
            )
            spec["file"] = str(found.relative_to(ROOT)).replace("\\", "/")
            specs.append(spec)
...
def process_sheet(spec: dict, results: dict, report: list[str], out_dir: Path):
    ...
    if layout == "4x4_action":
        ...
        action = spec["action"]
        name = spec["name"]
        entries = make_action_entries(action, name)
