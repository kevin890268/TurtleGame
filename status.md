...
- [x] **功能表**：右下角 ☰ 主按鈕打開圖示選單，選了之後展開那個區塊、其他收起來；每個區塊都能自己收合，狀態會記住；結構集中在 `js/menu.js` 的 `SECTIONS`
...
### 缸子與造景（目錄）
- [x] 列出 4 種缸子和 8 種造景的圖示與說明（`js/decor.js`），功能還沒做，標示「即將推出」
...
js/menu.js          功能表與可收合區塊（SECTIONS 決定順序和圖示）
js/decor.js         缸子與造景目錄
...
7. **養成內容**：缸子升級與造景的實際功能、零用錢與商店、成長相簿、更多隨機事件（打噴嚏、撿到保育類烏龜要通報）、提醒通知
