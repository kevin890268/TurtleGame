# 巴西龜（紅耳龜）姿勢圖：GPT 產圖提示詞

品種：**巴西龜（紅耳龜）**（red-eared slider）*Trachemys scripta elegans*。品種資料見 [docs/species.md](../../docs/species.md)、[docs/species_traits.md](../../docs/species_traits.md)。
...
1. **先做定裝照（第 0 步）**：上傳 `reference/sheets/bangui/sheet_a.png`（斑龜的姿勢表）當畫風參考，滿意後存成 `reference/sheets/slider/slider_ref.png`
2. 之後每張都在**新的對話**裡生成，上傳兩張圖：
   - `slider_ref.png`：角色參考
   - `reference/sheets/bangui/sheet_a.png`：畫風參考（斑龜的姿勢表，讓各品種畫風和大小一致）
3. 直接複製下面的完整提示詞
4. 存到 `reference/sheets/slider/`，檔名照各段標示的名稱，再執行 `python tools/slice_poses.py slider`

背景一律是**純洋紅色 #FF00FF**。GPT 的「透明背景」其實是半透明加雜訊，切圖工具處理洋紅色最乾淨。

---

## 第 0 步：定裝照｜`slider_ref.png`
...
Character: a young red-eared slider (Trachemys scripta elegans): a bright green carapace with yellow-green lines and swirls and a low keel, a yellow plastron with a dark round spot on each scute, green skin with yellow stripes, and a wide bright RED stripe behind each eye.
NOT a map turtle, NOT a Chinese stripe-necked turtle.
...
Attached images: (1) slider_ref.png is the EXACT character design; (2) the other image is the ART STYLE and layout reference only (do not copy that turtle's species).

Character: a young red-eared slider (Trachemys scripta elegans): a bright green carapace with yellow-green lines and swirls and a low keel, a yellow plastron with a dark round spot on each scute, green skin with yellow stripes, and a wide bright RED stripe behind each eye.
NOT a map turtle, NOT a Chinese stripe-necked turtle. Soft hand-painted storybook watercolor style with clean dark brown outlines.
...
Cells (left to right, top row then bottom row):
1. HAPPY AND HUNGRY: head raised toward the viewer, mouth open in a big begging smile, eyes sparkling, front feet paddling in the air with excitement
...
- [ ] 是巴西龜（紅耳龜）：小時候背甲是鮮綠色，有黃綠色的線條和漩渦紋，中線有低低的稜；腹甲黃色，每片上有深色圓斑
