...
> 產出一張 **3 × 4** 的圖，存成 `reference/sheets/bangui/action_13_flip.png`。
...
CONTEXT:
ABOVE WATER — the turtle lies UPSIDE DOWN: the CARAPACE rests on an invisible flat ground, the plastron faces up, the legs are in the air.

DESCRIPTION:
The turtle is stuck UPSIDE DOWN on its carapace and tries to turn itself back over.
...
IMPORTANT:
Follow rules section 29 (FLIP). Calm expression, no injury.
...
Seen from above and in front: the cream plastron faces the camera, head at the bottom of the cell reaching toward the viewer.

SIDE:
Turtle faces RIGHT, camera at roughly eye level. Never draw it facing left.
The shell in profile, upside down, head on the RIGHT.

BACK:
Behind the turtle, camera ABOVE looking DOWN about 45 degrees. Tail toward the viewer, face never visible.
Seen from above and behind: the cream plastron faces the camera, tail toward the viewer.
