# 密西西比地圖龜姿勢圖：GPT 產圖提示詞

品種：**密西西比地圖龜**（Mississippi map turtle）*Graptemys pseudogeographica kohnii*。品種資料見 [docs/species.md](../../docs/species.md)、[docs/species_traits.md](../../docs/species_traits.md)。
...
1. **先做定裝照（第 0 步）**：上傳 `reference/sheets/bangui/sheet_a.png`（斑龜的姿勢表）當畫風參考，滿意後存成 `reference/sheets/map/map_ref.png`
2. 之後每張都在**新的對話**裡生成，上傳兩張圖：
   - `map_ref.png`：角色參考
   - `reference/sheets/bangui/sheet_a.png`：畫風參考（斑龜的姿勢表，讓各品種畫風和大小一致）
3. 直接複製下面的完整提示詞
4. 存到 `reference/sheets/map/`，檔名照各段標示的名稱，再執行 `python tools/slice_poses.py map`

背景一律是**純洋紅色 #FF00FF**。GPT 的「透明背景」其實是半透明加雜訊，切圖工具處理洋紅色最乾淨。

---

## 第 0 步：定裝照｜`map_ref.png`
...
Character: a young Mississippi map turtle (Graptemys pseudogeographica kohnii): an olive-brown carapace with fine yellow net-like "map" lines on every scute and a dark blotch on each side scute; a raised central keel with a row of small dark-tipped KNOBS along the spine like a tiny mountain ridge; a SERRATED rear edge of the shell; a pale yellow plastron; olive-brown skin with many thin yellow stripes; and a distinctive yellow CRESCENT-shaped mark behind each eye.
NOT a red-eared slider (no red patch), NOT a Chinese stripe-necked turtle.
...
Attached images: (1) map_ref.png is the EXACT character design; (2) the other image is the ART STYLE and layout reference only (do not copy that turtle's species).

Character: a young Mississippi map turtle (Graptemys pseudogeographica kohnii): an olive-brown carapace with fine yellow net-like "map" lines on every scute and a dark blotch on each side scute; a raised central keel with a row of small dark-tipped KNOBS along the spine like a tiny mountain ridge; a SERRATED rear edge of the shell; a pale yellow plastron; olive-brown skin with many thin yellow stripes; and a distinctive yellow CRESCENT-shaped mark behind each eye.
NOT a red-eared slider (no red patch), NOT a Chinese stripe-necked turtle. Soft hand-painted storybook watercolor style with clean dark brown outlines.
...
1. WALKING mid-stride: the near front leg and the far hind leg step forward together (turtles move diagonal legs as a pair) while the other two push back; the shell is lifted clear of the ground, head forward and level, calm focused eyes
...
Cells (left to right, top row then bottom row):
1. BASKING, its favourite thing in the world: belly flat on the ground, neck stretched far forward and up toward the light, eyes closed in pure bliss, front legs spread wide to the sides, and BOTH HIND LEGS STRETCHED STRAIGHT BACKWARDS with the soles turned up toward the sky
...
2. STARTLED AND SHY: the head yanked halfway back into the shell, eyes wide open and worried, the body leaning back, front feet lifted slightly
...
Cells (left to right, top row then bottom row):
1. SWIMMING: body level, neck stretched forward, the near front leg in the middle of a strong stroke sweeping backward, the far front leg reaching forward, hind legs kicking; a streamlined, purposeful pose
...
5. BREATHING IN SHALLOW WATER: standing on the bottom with the body level, only the neck stretched straight up, nose pointing up as if poking the nostrils out of the water surface, calm eyes
...
- [ ] 是密西西比地圖龜：背甲橄欖色到褐色，每片盾片有黃色的網狀／橢圓形花紋（像地圖），側盾有深色斑塊；背甲中線有一排突起的稜（小時候特別明顯，像一排小尖角），後緣呈鋸齒狀
